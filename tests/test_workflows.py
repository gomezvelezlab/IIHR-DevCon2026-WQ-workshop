...
def test_hydrology_facade_solves_exports_and_loads(tmp_path) -> None:
    ...
    hydrology = Hydrology()
    hydrology.config(output_dir=output_dir, hours=3)
    ...
    cached = Hydrology()
    cached.config(output_dir=output_dir, hours=3)
...
def test_nitrogen_facade_loads_hydrology_solves_and_exports(tmp_path) -> None:
    ...
    hydrology = Hydrology()
    hydrology.config(output_dir=hydrology_dir, hours=4)
    hydrology.solve(progress=False)
    hydrology.export()

    nitrogen = Nitrogen()
    nitrogen.config(output_dir=nitrogen_dir)
    nitrogen.load_hydrology(hydrology_dir)

...
class Hydrology:
    ...
    def required_output_paths(self) -> list[Path]:
        return [
            self.output_dir / "discharge1.csv",
            self.output_dir / "states1.csv",
            self.output_dir / "fluxes1.csv",
            self.output_dir / "south_fork_aorc_forcing.csv",
        ]
    ...
    def export(self) -> dict[str, Path]:
        ...
        if self.result is None or self.forcing_df is None:
            if self.cache_exists():
                return {
                    "states": self.output_dir / "states1.csv",
                    "fluxes": self.output_dir / "fluxes1.csv",
                    "forcing": self.output_dir / "south_fork_aorc_forcing.csv",
                    "discharge": self.output_dir / "discharge1.csv",
                }
            raise RuntimeError("Hydrology.solve() must run before export().")
        return export_nitrogen_hydrology_inputs(self.result, self.forcing_df, self.output_dir)
    ...
    def load_outputs(self) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
        ...
        states = pd.read_csv(self.output_dir / "states1.csv", parse_dates=["time"])
        fluxes = pd.read_csv(self.output_dir / "fluxes1.csv", parse_dates=["time"])
        forcing = pd.read_csv(
            self.output_dir / "south_fork_aorc_forcing.csv",
            parse_dates=["time"],
        )

...
# Date: 2025-06-10
# Description: This module was coded by Jesus D. Gomez-Velez for interacting with AORC datasets.
# -----------------------------------------------------------------------------
...
class FileManagerAORC:
    ...
    **Specific Humidity, Pressure, Downward Radiation, Wind**
    ...
    def __init__(self, name: str, dir_output: str):
    ...
    def _download(self,
        ...
        dask.config.set(temporary_directory=self.dir_output)
        ...
        # Projection of the geometry to the AORC CRS epsg:4326
        if geometry_crs == "epsg:4326":
            geometry_projected = geometry
        else:
            geometry_projected = geometry.to_crs(epsg=4326)
        ...
        fname = f"aorc_{start_year}-{end_year}_{north_bound}x{east_bound}_{south_bound}x{west_bound}.nc"

        filename = os.path.join(self.dir_output, self.name + '_' + fname)
